...
- **발견된 버그 → ✅ 수정 완료 (2026-07-17)**: `/api/credits/history`가 오름차순+상한 500행이라 이력 500행 초과 사용자는 최근 이력이 누락되고 표시 잔액이 실제와 달라지는 문제(부수: 정확히 500행일 때 truncated=true 오탐)가 있었음. 수정: 잔액은 이력 목록과 분리해 `get_my_credit_balance()` RPC(전체 SUM, 기존 `/api/credits/balance`와 동일 RPC 재사용)로 별도 집계 → 거래 건수와 무관하게 항상 정확. 이력은 최신순(descending) + 상한보다 1행 더(501) 조회해 "정확히 상한"과 "상한 초과"를 구분(truncated 오탐 제거). `balanceAfter`는 정확한 현재 잔액에서 delta를 하나씩 빼며 과거로 역산. 테스트(`tests/credits/history.test.ts`)에 상한 초과(550건) 시나리오 추가해 회귀 방지 — 59/59 통과.
- 검증: `npm run test` 59/59, `tsc --noEmit` 클린, ESLint 신규/변경 파일 클린(19건 전부 선재 별건), `next build` 통과.
